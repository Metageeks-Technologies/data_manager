...
const reducer = (state, action) => {
  ...
  if (
    action.type === DELETE_DATA_SUCCESS ||
    action.type === MAKE_ACTIVITY_SUCCESS
  ) {
    return {
      ...state,
      isLoading: true,
  ...
  if (
    action.type === APPROVE_EDIT_SUCCESS ||
    action.type === REJECT_EDIT_SUCCESS ||
    action.type === MAKE_DATA_EDITABLE
  ) {
    return {
      ...state,
      toggleAction: !state.toggleAction,
