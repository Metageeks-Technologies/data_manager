...
const checkAllowedIP = catchAsyncError(async (req, res, next) => {
  // const clientIP = req.header('X-Forwarded-For');
  const clientIP = req.ip;
  const firstThreeOctets = clientIP.slice(0, clientIP.lastIndexOf(".") + 1);
  const regexPattern = new RegExp(
    `^${firstThreeOctets}(24[0-3]|25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])$`
  );
  ...
  if (foundIP || clientIP === "::1") {
    ...
    return next(
      new ErrorHandler(`${clientIP} This ip is not whitelisted`, 403)
    );
  }
});
